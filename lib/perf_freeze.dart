...
void _workerIsolateFun(SendPort workerSendPort)  async {
  ...
  while (true) {
    ...
    if (!syncCompleter.isCompleted) {
      ...
      (monitor ??= NativePerfFreezeMonitor(Platform.isAndroid? DynamicLibrary.open('libnative_freeze.so') : DynamicLibrary.process())).uiThreadStacktrace(pointer, stackDepth);
